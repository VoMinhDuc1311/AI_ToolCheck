 ...
 *
 * <p>Điều phối các lời gọi LLM theo chiến lược ưu tiên 3 tầng (Tiered Fallback):
 * <ol>
 *   <li><b>Tier 1 — Primary (Local):</b> Ollama với model {@code qwen3-coder:30b} (chất lượng cao nhất).</li>
 *   <li><b>Tier 2 — Fallback (Local):</b> Ollama với model {@code qwen2.5-coder:7b} (nhanh hơn, nhẹ hơn).</li>
 *   <li><b>Tier 3 — Last Resort (Cloud):</b> Google Gemini API. Có thêm 15s throttle để tránh Rate Limit 429.</li>
 * </ol>
 *
 * <p>Nếu tất cả 3 tier đều thất bại, ném ra {@link AiJsonParseException} với
 * {@link ErrorType#INVALID_JSON_SYNTAX} để Consumer đánh dấu Job là FAILED
 * và lưu Audit Log.
 *
 * <p><b>Thiết kế quan trọng:</b> Không có {@code Thread.sleep} cho Ollama Local.
...
public class AiModelRouterService {
    ...
    /**
     * Thực thi prompt qua chuỗi fallback 3 tầng và trả về raw text từ LLM đầu tiên thành công.
     *
        ...
        try {
            ...
            try {
                ...
                com.aitoolcheck.ai_toolcheck1_backend.dto.gemini.res.GeminiResponse geminiResponse = 
                        geminiApiClientService.getFullAiResponse(prompt);
